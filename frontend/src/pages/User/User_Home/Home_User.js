...
//       });
//     } else {
//       // Fetch all popups and categorize them for non-logged in users
...
//       .then((res) => res.json())
//       .then(res => {
//         console.log('Received locations:', res);
//         if (res.data && Array.isArray(res.data)) {
//           setLocations(res.data);

//           // Categorize popups
//           const categorizedPopups = res.data.reduce((acc, location) => {
//             location.categories.forEach(category => {
//               if (!acc[category.category]) {
//                 acc[category.category] = [];
//               }
//               acc[category.category].push(location);
//             });
//             return acc;
//           }, {});

//           setCategories(Object.keys(categorizedPopups).map(category => ({
//             title: category,
//             popups: categorizedPopups[category]
//           })));
//         } else {
//           console.error("Received data does not contain an array of locations:", res);
//           setLocations([]);
//         }
//         setLoading(false);
//       })
...
//             {category.popups && category.popups.length > 0 ? (
//               <div className='lower-popup-wrapper'>
//                 {category.popups.map((popup, popupIndex) => (
//                   <Link key={popupIndex} to={`/popup/user/detail/${popup.id}`} className='lower-popup-link'>
//                     <div className="lower-popup-item">
//                       <div className="lower-popup-image-box">
//                         {popup.popupImages && popup.popupImages.length > 0 ? (
//                           popup.popupImages.map((image, imageIndex) => (
//                             <img
//                               key={imageIndex} 
...
//                         <h3>{popup.title}</h3>
//                         <p>{popup.startDate ? `${popup.startDate} ~ ${popup.endDate}` : '상시 운영'}</p>
//                         <div className="lower-category-box">
//                           {popup.categories && popup.categories.map((popupCategory, index) => (
//                             <div key={index} className="lower-category-item">{popupCategory.category}</div>
//                           ))}
//                         </div>
//                       </div>
//                     </div>
